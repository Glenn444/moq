...
```bash
ffmpeg -i input.mp4 -f mp4 -movflags cmaf - | \
    moq --listen '[::]:4443' --tls-generate localhost --broadcast my-stream.hang import fmp4
```
...
```bash
moq --listen '[::]:4443' --tls-generate localhost --broadcast my-stream.hang export fmp4 | ffplay -
```
