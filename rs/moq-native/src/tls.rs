...
pub struct Listen {
	...
	#[arg(long = "tls-cert", id = "tls-cert", env = "MOQ_LISTEN_TLS_CERT")]
	#[serde(default, skip_serializing_if = "Vec::is_empty")]
	#[serde_as(as = "serde_with::OneOrMany<_>")]
	pub cert: Vec<PathBuf>,

	/// Load the given key from disk.
	#[arg(long = "tls-key", id = "tls-key", env = "MOQ_LISTEN_TLS_KEY")]
	#[serde(default, skip_serializing_if = "Vec::is_empty")]
	#[serde_as(as = "serde_with::OneOrMany<_>")]
	pub key: Vec<PathBuf>,
...
impl Listen {
	...
	pub fn resolved(&self) -> Self {
		for (used, deprecated, canonical) in [
			(!self.deprecated.cert.is_empty(), "--server-tls-cert", "--tls-cert"),
			(!self.deprecated.key.is_empty(), "--server-tls-key", "--tls-key"),
			(
...
mod legacy_tests {
	...
	/// The served identity kept its flag names but not its env vars, so the hidden
	/// `--server-tls-*` args are what keep `MOQ_SERVER_TLS_*` working.
	#[test]
