...
/// The released env vars for this section, which moved from `MOQ_CLIENT_*` to
/// `MOQ_CONNECT_*` with the rest of the dial side.
///
/// The flags kept their names, so these exist only to give the old variables
/// somewhere to land: clap allows one env var per arg.
#[derive(Clone, Debug, Default, clap::Args)]
#[group(id = "websocket-legacy")]
pub(crate) struct Legacy {
	#[arg(
		id = "client-websocket-enabled",
		long = "client-websocket-enabled",
	...
	#[arg(
		id = "client-websocket-delay",
		long = "client-websocket-delay",
...
/// The QUIC head start when `--websocket-delay` is unset.
...
impl Config {
	...
	pub fn resolved(&self) -> Self {
		...
		{
			tracing::warn!("MOQ_CLIENT_WEBSOCKET_ENABLED is deprecated; use MOQ_CONNECT_WEBSOCKET_ENABLED");
			resolved.enabled = Some(enabled);
		}
		if self.delay.is_none()
			&& let Some(delay) = self.legacy.delay
		{
			tracing::warn!("MOQ_CLIENT_WEBSOCKET_DELAY is deprecated; use MOQ_CONNECT_WEBSOCKET_DELAY");
...
mod legacy_tests {
	...
	fn released_spellings_fold_in() {
		let config = parse(&["--client-websocket-enabled=false", "--client-websocket-delay", "1s"]).resolved();
		assert!(!config.resolved_enabled());
		assert_eq!(config.resolved_delay(), time::Duration::from_secs(1));

		// The canonical spelling wins.
		let config = parse(&["--websocket-delay", "2s", "--client-websocket-delay", "1s"]).resolved();
